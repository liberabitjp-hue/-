...
export async function checkFileBeforeParse(file: File): Promise<FileCheckResult> {
  if (file.size === 0) {
    return { ok: false, reason: '空のファイルです（サイズが0バイト）。中身のあるExcelファイルを選択してください。' }
  }
...
export function checkParsedWorkbook(sheetCount: number): FileCheckResult {
  if (sheetCount === 0) {
    return { ok: false, reason: 'このファイルにはシートが1つもありませんでした。対象のシートが含まれるファイルか確認してください。' }
  }
 ...
 *  Japanese explanation. Falls back to including the original message so a
 *  genuinely new failure mode is still diagnosable, just not user-friendly. */
export function describeParseError(err: unknown): string {
  const message = err instanceof Error ? err.message : String(err)
  const lower = message.toLowerCase()

  if (lower.includes('password') || lower.includes('encrypt')) {
    return 'このファイルはパスワードで保護されているため読み込めません。パスワードを解除してから取り込んでください。'
  }
  if (lower.includes('corrupt') || lower.includes('central directory') || lower.includes('zip')) {
    return 'ファイルが破損している可能性があります。Excelで開き直して保存し直すか、別のファイルでお試しください。'
  }
  if (lower.includes('unsupported') || lower.includes('unrecognized') || lower.includes('cannot find')) {
    return 'この形式は読み込みに対応していません。Excel形式（.xlsx/.xlsm/.xls）で保存し直してください。'
  }
  return `読み込み中に問題が発生しました（詳細: ${message}）。ファイルが壊れていないか確認するか、別のファイルでお試しください。`
}
